...
if ($result->num_rows > 0) {
    ...
    echo "<table border='1'>";
    echo "<tr><th>Course Name</th><th>Description</th><th>Remaining Capacity</th></tr>";
    while ($row = $result->fetch_assoc()) {
        $remaining_capacity = $row["max_capacity"];
        $course_id = $row["course_id"];
        $course_name = $row["course_name"];
        $description = $row["description"];

        // Count enrolled students
        $sql_count = "SELECT COUNT(*) AS enrolled FROM students WHERE course_id = $course_id";
        $result_count = $conn->query($sql_count);
        if ($result_count->num_rows > 0) {
            $row_count = $result_count->fetch_assoc();
            $enrolled = $row_count["enrolled"];
            $remaining_capacity = $row["max_capacity"] - $enrolled;
        }

        echo "<tr><td>$course_name</td><td>$description</td><td>$remaining_capacity</td></tr>";
    }
    echo "</table>";
} else {
            ...
            <nav class="navbar navbar-expand-md navbar-light bg-faded">
                ...
                <div class="collapse navbar-collapse main-menu" id="navbarSupportedContent">
                    <ul class="navbar-nav nav lavalamp ml-auto menu">
                        ...
                        <li class="nav-item"><a href="course.php" class="nav-link active">Courses</a>
